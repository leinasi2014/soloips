...
- (e) `data-contract.md` §4.2 的目标设计含「**创建公司时自动创建默认总助理任职**」（`createDefaultGeneralAssistant`），与用户「总助理由招募产生」的表述**不一致**。是否**放弃该自动创建设计**？（该设计本就未实现，故不涉及迁移，只需确认方向）
  - **〔已裁定，2026-09-18 关闭〕**：**放弃该自动创建设计**。`data-contract.md` 已**删除**该示例（§4.2 现为「待设计伪代码」占位），并新增 §2.4.1「创建后『待招募』状态合法」与 §2.4.2「可信用户入口：招募首个总助理」——公司创建后**无总助理为合法中间状态**，首任总助理由**用户**经可信入口招募。本问题由指挥裁定关闭。
