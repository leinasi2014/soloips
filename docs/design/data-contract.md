...
**实测可达的序列**（BE-4c 写手用 10–12 个悬挂团队使目标排最后，在 reconcile 的写循环期间经**纯公开命令**完成恢复）：
