...
| 写入形态为 `update`（读-改-写），不是整条 `put` | **只保证字段级不误伤**（不改其他字段）。**不等于「不会写入陈旧判定」**——见 P-10.5 / P-10.6 |

**P-10.5 实测修正（2026-09-18，BE-4c 发现）**〔约束〕：

上表最后一行原写作「不会用陈旧快照覆盖其他字段」，**该推论过强、已被实测推翻**——`status: "inactive"` 的取值本身是**扫描时刻的判定结论**，与写入时刻的实际状态无关。

**实测可达的序列**（构造方式：多个悬挂团队使目标排在写循环末位，在 reconcile 的写循环期间经**纯公开命令**完成恢复）：

```text
① 团队 A 处于「active 但组长已撤销」（P-8 崩溃残留）
② reconcileTeams 扫描 → 把 A 列入待置 inactive（判定基于 ①）
③ 写循环进行中（同步扫描已结束、异步写尚未全部完成）
④ 经公开命令恢复 A：createAppointment(team_lead) + activateTeam → A 重新可用
⑤ reconcile 写到 A → 以 ② 的陈旧结论覆盖 → A 变回 inactive、usable=false，
   尽管其 leadReference.valid === true
```
...
| 项 | 结论 |
|---|---|
| 是否违反 P-10.1 三条补偿 | **不违反**（写权复核、幂等收敛、读面不依赖标记均仍成立） |
| 危险方向 | **fail-closed**：覆盖后团队不可用，**不会**让失效团队被读作可用 |
| 代价 | 一次**合法的、已提交的**激活被陈旧判定覆盖 → 该团队需经 `activateTeam` **重新激活** |
| 窗口 | 仅在 `reconcileTeams` 执行写循环期间（同步扫描已完成、异步写未完成） |

**裁定**：

| # | 规则 |
|---|---|
| **P-10.5.1** | ~~**当前形态在 M0.1 仍可接受**——代价是「假不可用 + 可经公开命令恢复」，非数据损坏、非越权、非 fail-open。~~ **〔2026-09-18 外审 F-01 推翻，见 P-10.6〕本条的接受结论已撤回**：该接受只考虑了「激活被覆盖」这一条出边，未检查**并发归档**——反例已实测（归档终态被改回 `inactive`）。**正确表述**：本条描述的**激活被覆盖**情形可经 `activateTeam` 恢复；但同一机制还可破坏 `archived` 终态，那**不可经公开命令恢复**（`archived` 无出边、`activateTeam` 对 archived 抛 PRECONDITION） |
| **P-10.5.2** | **`reconcileTeams` 与正常写入可交错**是本裁定确认的事实；调用者须知道「跑 reconcile 可能与在途恢复操作竞争」 |
| **P-10.5.3** | **收紧路径（原 P-10.3）升级为「建议在 M0.2 处理」**：① 让 reconcile 经提交门（登记 `team.reconcile` kind）；或 ② 改为「写前重核 P-4」的 CAS 语义（写入时重新判定而非用扫描结论）。两者都是**独立切片**，不在 M0.1 强制 |
| **P-10.5.4** | ~~**不得**把本修正读成「reconcileTeams 不安全」——它是**已量化的、fail-closed 的**已知代价。~~ **〔2026-09-18 外审 F-01 + 本仓实测：撤回〕** 该「不得称其不安全」的措辞**不得**再用于压制对更严重出边的报告：反例已证明同一机制可破坏 `archived` 终态。**已在 P-10.6 处置，不再是「已知可接受代价」** |

**〔待实现〕**：P-10.5.3 的两条收紧路径均未实施。**在收紧前，不得声称「core 内全部写都经提交门」，也不得声称「reconcileTeams 与正常写入不会竞争」。**

**P-10.6 `reconcileTeams` 陈旧判定——并发归档出边与槽位内重判修复（2026-09-18，外审 F-01 + 本仓实测；切片 `fix/F-01-reconcile-exclusive`）**〔约束〕：

**外审发现的更强反例**（静态审查 + **本仓可控 barrier 实测复现**）：P-10.5 只检查了「激活被覆盖」这一条出边。同一机制还有第二条出边——**已提交的归档终态被改回 `inactive`**：

```text
① 团队 A 处于「active 但组长引用悬挂」（P-8 崩溃残留）
② reconcileTeams 同步扫描 → 把 A 列入待置 inactive（判定基于 ①）
③ 窗口：经提交门提交一次正常 closeTeam(A) → A = archived（终态，P-7）
④ reconcile 的写循环随后执行 → 用 ② 的陈旧结论覆盖 → A = inactive
```

**为何比 P-10.5 严重**：`archived` 是 P-7 的**终态、无出边**；`activateTeam` 对 `archived` 抛 `SOLOIPS_CORE_PRECONDITION`（门内有显式判定）。被改回 `inactive` 之后，该团队**可被再次激活**——即「归档终态」这一约束实际失效，且**不可经公开命令恢复**（没有「变回 archived」的入口）。故 P-10.5.1 的「可经公开命令恢复」不适用于本条。

**复现纪律（本仓实测，非静态推断）**：`packages/core/tests/reconcile-atomicity.spec.ts` 用**可控 barrier** 拦在 `#reconcileOne` 的第一个 await（`lease.assertHeld`）——此刻「同步扫描已完成、写入未发生」。barrier 的开关由测试显式控制，**不依赖 sleep、不依赖团队数量 N、不依赖微任务预算**。修复前该用例转红（`expected 'archived' to be 'inactive'`）。

**修法（已实施并验证）**：判定移入**介质写链槽位内**——`update` 的 `revise` 回调里重判两项：① `current.status === "active"`；② 当前组长引用**仍**失效。判定不成立即**中止本次写**（抛私有信号 → 端口不调用后端 `putRecord`，**零介质写**），且**不报告**该 id 为「本次标记」。

**原子性依据**（**已核到宿主实现**，`@deepseek-ai/dsh-storage-domain` 的 `lib/index.js`，非推断）：`DomainImpl` 为**整个 domain**（全部表共用）维护唯一写链 `chain`，每次 `put`/`update`/`delete` 经 `enqueue(job)` 排入该链；`KvTableImpl.update` 在槽位内**同步**执行 `fn(records.get(key))`，成功后才 `await unit.putRecord(...)`。故「重判 + 写入」与提交门的每次发布占**同一串行边界**，本地无第二个提交可插入；跨进程仍由 writer lease 排除。

**由此撤回与限定**（对照外审要求）：

| # | 规则 |
|---|---|
| **P-10.6.1** | **P-10.5.1 / P-10.5.4 的接受与「不得称其不安全」措辞撤回**；`reconcileTeams` 的并发写缺口在 M0.1 **必须处置**（已处置，见本节的修法） |
| **P-10.6.2** | **不得**再把 N（团队数量）、微任务预算或「多跑几次」写成正确性条件——反例的构造是**决定性的 barrier 控制**，与实现成本无关 |
| **P-10.6.3** | 该修复**不改变**「不经提交门」的既有登记（P-10.1/P-10.3 的收紧路径仍留待后续切片）；它补的是**写前重判**这一条必要条件，不是门的替代 |
| **P-10.6.4** | **每处绕门写入都须在状态机的全部出边上验证**，不是发现一个 fail-closed 例子就判整个机制 fail-closed（外审：原 P-10 只验证了恢复激活、未检查并发归档） |
| **P-10.6.5** | 修复的回归保护：`tests/reconcile-atomicity.spec.ts` 的用例必须在**「槽位外重读 + 无条件 update」这一形态**下转红——否则该防线强度无回归保护 |

**〔新发现，未处置〕**：本仓修复过程中发现 `#reconcileOne` 的 catch 若写成「无条件 `return false`」会**静默吞掉后端故障**（reconcile 谎报「扫描完成、无事可做」）。当前实现是「只识别私有信号、其余重抛」，但该行为的**回归测试**属本轮返工补充项。
...
| `inconsistent` | 存在**多于一条**有效任职（§2.4.2 唯一性被破坏） | 独立分支 + **全量清单**，**不取首条掩盖**（BE-4a 实现裁定：写面/迁移缺陷必须可见） |

**查询失败不在此形状内**：失败以**抛出的错误**呈现（`SOLOIPS_CORE_STORE_CLOSED`/`SOLOIPS_CORE_RECORD_INVALID`/介质错误），**不编码为分支**——把失败编码成返回值会让调用方把「读不到」误读成「没有」。**本投影的每个分支都表示查询已成功执行。**（BE-4a 实现者裁定，经独立 QA 三轮核验。）
...
| **只读投影边界** | **不得含 `accountId`**（BE-6a 落地时裁定，2026-09-18）——见下方裁定四.1 |

**裁定四.1：只读投影不得携带 `accountId`（2026-09-18，BE-6a 落地时裁定）**〔约束〕：

`SoloipsCompanyRecord` 含 `accountId`（§2.1 字段表），但**发给浏览器的只读投影必须剥离它**。依据：

1. **它是裁定四第一行的直接推论**——「不由任何 UI 或模型参数传入/覆盖」若同时把该值**发给**浏览器，界面就有机会写出「按 `accountId` 判断」的界面侧授权；那正是本裁定要消除的形态。**不暴露强于「暴露但约定不要用」**（后者正是外审批评的「靠约定代替入口拒绝」）。
2. **账户绑定是部署层事实**：一个数据根只绑一个账户（§3.1），界面无法对它采取任何有意义的行动。
3. **本仓已有技术强制**：`tsdown.config.ts` 的 `client-artifact-contract` 把 `"accountId"` 列入浏览器产物**禁止标识**（判据为产物字节子串匹配），原样透传会使构建失败。**不得**为通过而放宽该清单——正确的处置是在**边界投影**上剥离。

**实现约束**：剥离须在**唯一的出口**完成（如 `companyViewOf()`），`getCompany` 与 `getCompanyTree` 共用，避免多处各自剥离产生漂移；并须有测试断言**该键不存在**（不是「值为 `undefined`」）且其余字段逐字保留。

**影响面**：后续所有面向浏览器的只读投影（FE-2/FE-3 及 BE-6b 的查询面）同受本条约束。
...
**裁定六：M-A-ASM——装配层身份一致性（2026-09-18 补，来自外审 Q2 + BE-6a 实测）**〔约束〕：

**缘起（实测）**：BE-6a 的真实浏览器 E2E 报 `soloips/createCompany → gateway/internal / "Receiver must be an instance of class SoloipsWebHost"`。根因是产物含**两份** `SoloipsWebHost` 类定义（`lib/index.js` 由 tsdown 打包、`lib/types/index.js` 由 tsc 编译出运行期代码），网关注册与实例分属不同副本。该缺陷**单测、静态门禁、乃至 required CI 都抓不到**（444 用例全绿、`verify` job = success，而同 commit 的 message 已写明业务 E2E 失败）。

**条款**（**行为层，不绑定实现**）：

> 候选的**实际交付产物**在干净 DSH 实例装配后：
> 1. 所有 M-A 所需 public Remote 必须经**真实浏览器 / gateway** 到达**正确的 Host service instance**；
> 2. 候选产物身份必须**可核对**（digest/SHA 与本次候选一致，非旧产物）；
> 3. `create → read → refresh/restart → read` 必须成立；
> 4. **不得**以 `gateway/internal`、receiver identity mismatch、或加载旧候选的方式「通过」。

**刻意不写**（避免过拟合实现）：**不**规定「系统内只能有一个 `SoloipsWebHost` class」——那是实现细节；若未来改为 factory / proxy / 其他注册方式，本条款仍须成立。

**与既有条目的关系**：本条款是 §13 M-A 定义的**装配层可验证化**（非替代），是 Z-02（浏览器动作 + 网络 + Host 结果 + 版本/digest 一致）与 Z-03（实际创建与持久读回）的形式化，也是 §5.2.2 完成条件第 4 条（最终 CI 与适用的真实运行证据有效）在 M-A 类切片上的具体化。

**防假绿约束**：

- **脚本存在 ≠ 已执行 ≠ 已通过**——三者必须区分（本项目已因 `test:artifacts` 学过一次）。
- 验收证据必须**与候选摘要绑定**且**可被外部 reviewer 获取**（不得只留本机路径）。
- 若某环境（如 GitHub-hosted runner）无法运行真实消费者链路，**不得伪造**可跑步骤；应做「**证据缺失即显性失败**」的 fail-closed 门，而不是让「没跑」被当成「通过」。
...
| 2026-09-18 | **BE-4a 交付同步**：①§0 能力面行更新为 BE-4a 后状态（读面 5 方法全部落地、`listTeams` 补 `departmentId` 过滤、**读面缺口：无**、服务面行号改 `contracts.ts:1110-1345`）；②§2.4.1 SA-01 段从〔待实现〕改〔已实现〕——`listAdministrators` 四态显式投影（`vacant` 显式空数组 / `uncovered` 不携带字段 / `present` / `inconsistent` 全量不掩盖）+ **失败以抛错呈现、不编码为分支**（映射 SA-01.1~04）；③补记读面统一校验收紧（6 个 `get*` 非法 id 由静默 `undefined` 改为抛 `SOLOIPS_CORE_VALIDATION`；`operationId` 按 `isOperationIdShape` 校验，依据 `ids.ts` 既定契约） | BE-4a 交付（Issue #21/#31）；实现裁定经独立 QA 三轮核验（含 D1 缺陷修复、3 项盲区补测、真实 SQLite 介质验证） |
| 2026-09-18 | **P-10.5 实测修正（BE-4c 发现）**：原「写入形态为 `update` ⇒ 不会用陈旧快照覆盖」的推论**过强、被实测推翻**——`status: "inactive"` 是**扫描时刻的判定结论**，与写入时刻状态无关。实测序列（多团队使目标排写循环末位 + 在写循环期间经公开命令恢复）可让**一次合法的、已提交的激活被陈旧判定覆盖**，团队变回不可用（`leadReference.valid === true` 但 `usable === false`）。**性质**：不违反 P-10.1 三条补偿、方向 **fail-closed**（不会让失效团队被读作可用）、代价为「假不可用 + 可经 `activateTeam` 恢复」。**裁定**：M0.1 仍可接受但不得再作原声称；收紧路径（经门 或 写前重核 P-4 的 CAS）升级为**建议 M0.2 处理** | BE-4c 写手按停止条件停手报告；指挥裁定并修订契约 |
| 2026-09-18 | **P-10.5 精度修订（QA 独立复现发现）**：QA 按 N 扫描发现复现阈值依**构造成本**而变——N=5/10/12/20 未复现（恢复先于目标写入完成），N=25 复现，**N≥30 稳定复现**（N=30/35/40 各 3 次一致）。差异源于「恢复命令 vs 写循环」的微任务预算对比取决于恢复路径成本。故正文不再写死 N（改为「多个悬挂团队使目标排在写循环末位」）——**缺陷性质与方向不变**（已由 QA 在基线与候选双向复现），仅构造参数依实现而变。 | QA 独立复现 BE-4c 时发现；指挥修订 |
| 2026-09-18 | **P-10.6 新增：`reconcileTeams` 并发归档出边 + 槽位内重判修复（外审 F-01）**——外审静态反例经**本仓可控 barrier 实测复现**：扫描完成、写入前插入一次正常 `closeTeam`（提交为 `archived`）后，reconcile 把 **归档终态改回 `inactive`**（P-7 被绕门写入破坏，且不可经公开命令恢复）。**撤回** P-10.5.1/P-10.5.4 的「M0.1 可接受」与「不得称其不安全」措辞；**修法**：判定移入介质写链槽位内（`update` 的 `revise` 重判 status + 组长引用，不成立即中止写入=零介质写）；**原子性依据核到宿主实现**（`DomainImpl` 唯一写链 `chain` / `KvTableImpl.update` 同步 `fn` 后落盘）。新增纪律 P-10.6.2（不得把 N/微任务预算写成正确性条件）、P-10.6.4（每处绕门写须在**全部出边**上验证） | 外审 F-01（Issue #35）+ 本仓实测复现；切片 `fix/F-01-reconcile-exclusive` |
| 2026-09-18 | **新增 §2.5.1 裁定四.1「只读投影不得携带 `accountId`」（BE-6a 落地时裁定）**：① 它是裁定四第一行的直接推论（暴露给浏览器会诱导界面侧授权）；② 账户绑定是部署层事实，界面对其无可行动；③ 本仓 `tsdown.config.ts` 的 `client-artifact-contract` 已把 `"accountId"` 列为浏览器产物禁止标识，**不得**为通过而放宽，须在边界投影剥离；④ 剥离须在唯一出口完成并有「键不存在」断言（非「值为 undefined」）；⑤ 后续所有面向浏览器的只读投影同受约束 | 指挥裁定（BE-6a 写手实测发现门禁冲突并拒绝放宽既有安全门，按纪律上报）；依据 `data-contract.md` §2.5.1 裁定四 + `tsdown.config.ts:129/:252` |
| 2026-09-18 | **新增 §2.5.1 裁定六「M-A-ASM——装配层身份一致性」**：① 缘起为 BE-6a 实测的 `Receiver must be an instance of class SoloipsWebHost`（产物含两份类定义；该缺陷单测 / 静态门禁 / required CI 均抓不到）；② 条款为**行为层、不绑定实现**（真实浏览器经 gateway 到达正确 Host 实例 + 产物身份可核对 + create→read→refresh/restart→read），**刻意不规定**「只能有一个 class」；③ 与 §13 M-A、Z-02/Z-03、§5.2.2 第 4 条的关系明确；④ 防假绿：脚本存在 ≠ 已执行 ≠ 已通过；证据须与候选摘要绑定且外部可读；无法运行真实链路时做 fail-closed 证据门，不伪造步骤 | 外审 Q2 答复 + BE-6a 实测；指挥核实构建配置与产物身份后落档 |
