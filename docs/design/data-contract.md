...
| 写入形态为 `update`（读-改-写），不是整条 `put` | **只保证字段级不误伤**（不改其他字段）。**不等于「不会写入陈旧判定」**——见下方 P-10.5 的实测修正 |
...
| # | 规则 |
|---|---|
| **P-10.5.1** | **当前形态在 M0.1 仍可接受**——代价是「假不可用 + 可经公开命令恢复」，非数据损坏、非越权、非 fail-open。但**不得**再声称「`update` 形态保证不会写入陈旧判定」 |
...
| **P-10.5.4** | **不得**把本修正读成「reconcileTeams 不安全」——它是**已量化的、fail-closed 的**已知代价，且已在本节显式登记 |
