...
1. **它是裁定四第一行的直接推论**——「不由任何 UI 或模型参数传入/覆盖」若同时把该值**发给**浏览器，界面就有机会写出「按 `accountId` 判断」的界面侧授权；那正是本裁定要消除的形态。**不暴露强于「暴露但约定不要用」**。
...
**实现约束**：剥离须在**唯一的出口**完成（如 `companyViewOf()`），`getCompany` 与 `getCompanyTree` 共用；并须有测试断言**该键不存在**（不是「值为 `undefined`」）且其余字段逐字保留。
...
| 2026-09-18 | **新增 §2.5.1 裁定四.1「只读投影不得携带 `accountId`」**：① 它是裁定四第一行的直接推论（暴露给浏览器会诱导界面侧授权）；② 账户绑定是部署层事实，界面对其无可行动；③ 本仓 `tsdown.config.ts` 的 `client-artifact-contract` 已把 `"accountId"` 列为浏览器产物禁止标识，**不得**为通过而放宽，须在边界投影剥离；④ 剥离须在唯一出口完成并有「键不存在」断言（非「值为 undefined」）；⑤ 后续所有面向浏览器的只读投影同受约束 | 指挥裁定（BE-6a 写手实测发现门禁冲突并拒绝放宽既有安全门，按纪律上报）；依据 `data-contract.md` §2.5.1 裁定四 + `tsdown.config.ts` 的禁止标识清单 |
