/**
 * SOLOIPS-ADAPTER-CONTRACTS（T02 契约草案 · 编译探针副本）
 *
 ...
 *
 * 编译验证：本文件在 DEV-02 严格基线下通过类型检查（见 compile-receipt.md P1/P8）。
 *
...
export type SoloipsSessionLogOffset = number & { readonly [soloipsBrand]: 'SoloipsSessionLogOffset' }
...
/** 从表声明中取出键类型。 */
export type SoloipsTableKeyOf<
  S extends SoloipsDomainSpec,
  N extends keyof S['tables'],
> = S['tables'][N] extends SoloipsDomainTableSpec<infer K> ? K : never

/** 从表声明中取出记录类型。 */
export type SoloipsTableValueOf<
  S extends SoloipsDomainSpec,
  N extends keyof S['tables'],
...
export type SoloipsDomainGlobalHandleOf<S extends SoloipsDomainSpec> =
  S extends { readonly global: SoloipsDomainGlobalSpec<infer G> } ? SoloipsDomainGlobal<G> : never
...
/** 一次子智能体运行的终态原因。 */
export type SoloipsSubagentStopReason = 'completed' | 'aborted' | 'error' | 'max-tokens' | 'refusal'
...
export interface SoloipsSubagentPort {
  ...
  listChildren(parentSessionId: SoloipsSessionId, signal?: AbortSignal): Promise<readonly SoloipsSubagentListEntry[]>
  /** 枚举根会话的完整子智能体树（稳定前序）。 */
  listDescendants(rootSessionId: SoloipsSessionId, signal?: AbortSignal): Promise<readonly SoloipsSubagentListEntry[]>
 ...
 */
export type SoloipsToolGuard = (exec: SoloipsToolRunContext & { readonly name: string }) => string | undefined
...
export interface SoloipsEventMap {
  ...
  /** 一个子智能体运行被发布。 */
  'soloips:subagent/start': (info: { readonly runId: string; readonly childId: SoloipsSessionId; readonly provider: string }) => void
  /** 一个已发布的子智能体运行已结算。 */
  'soloips:subagent/end': (info: { readonly runId: string; readonly childId: SoloipsSessionId; readonly result: SoloipsSubagentResult }) => void
...
export interface SoloipsEventsPort {
  ...
  /** 并行派发并等待全部监听。 */
  parallel<K extends keyof SoloipsEventMap>(name: K, ...args: Parameters<SoloipsEventMap[K]>): Promise<void>
  /** 串行派发；首个抛出即中断后续。 */
  serial<K extends keyof SoloipsEventMap>(name: K, ...args: Parameters<SoloipsEventMap[K]>): Promise<void>
}
...
export interface SoloipsTeamPort {
  ...
  sendMessage(lead: SoloipsAgentRef, target: string, content: readonly SoloipsContentBlock[]): Promise<string>
  createTask(lead: SoloipsAgentRef, subject: string, description: string): Promise<SoloipsTeamTaskView>
  updateTask(lead: SoloipsAgentRef, taskId: string, action: 'claim' | 'complete'): Promise<SoloipsTeamTaskView>
}
