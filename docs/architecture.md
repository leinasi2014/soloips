...
**〔已确认，SOLO-03〕** 第一条业务验收选择短剧：先完成 IP 简纲，再测试生成可观看的 PV。PV 是试水宣传短片，首片不等于完整短剧制作与发行完成。

**〔产品愿景，来自 V0.4〕** 业务主线是“原创 IP 孵化 → 试水作品 → 自营或合作发行 → 数据反馈 → 扩展、重构或资产复用”。小说、漫画、游戏、音乐及视频属于产品的创作与发行方向。第一条交付路径不会自动取消其余方向，也不等于整套商业生态已完成。
...
flowchart TB
    ...
    P --> C
    C --> T[协作内核：任务、尝试、提交、审核、接续]
    T --> D[DSH：Agent、Session、模型适配、工具、持久化扩展]
    D --> M[模型 API 与获准的创作工具]
    R[RLM 工具插件] --> D
```
...
| 协作内核 | Team 任务、attempt、依赖、提交、审核、消息与调度 | 从现有插件筛选实现并改造成 SoloIPs 内部模块；一个任务保留一个权威状态来源 |
...
DSH 本地还有实验性的 Agent Team 能力，具有任务板和持久消息。其适用范围是同一工作目录内的小团队，文档明确不支持成员独立工作目录、跨进程共管或自动释放任务 owner；不能据名称把它等同于本项目的长期公司、入职和独立审核。首片优先复用 swarm 的任务/审核语义；是否替换某个内部机制，应以实际差异和验证决定，不能同时维护官方 Team 与 swarm 两份任务权威。
...
**〔沿用重构要求，SOLO-A02〕** 长期组织身份、一次协作和一次执行分别建模。
...
**〔迁移建议，SOLO-A03〕** 采用“按能力接入、逐片验证”。不将当前插件目录整体复制改名后宣布重构完成，也不要求重写已有合格能力。

| 来源 | 本次核实的状态 | 接入处理 |
| --- | --- | --- |
| `repair-release` | 现有 Team 协作与 Web 插件候选；仍有恢复写入/schema、待回答、停线与接续缺口 | 作为协作实现来源，按首片依赖移入并验证 |
...
**〔已确认范围〕** AI 公司接受目标，员工协作交付，用户最终验收；首版使用 DSH Web。

**〔已确认样例〕** 短剧 IP 简纲与第一支 PV。先产出简纲，确定故事钩子、主要人物和 PV 想验证的观感，再形成镜头计划、参考素材、镜头视频和可播放的合成片。首支片的时长、画幅、画风、声音和具体生成工具在制作任务启动前收敛；当前不假定已有视频服务可调用。
...
**〔用户确认，2026-09-15〕** 本轮先组织文档团队，协助理清产品、重构架构与验收要求。下表替换此前的模型分配建议；它是建队配置要求，不代表团队已创建或成员已经执行。
