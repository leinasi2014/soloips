...
### 1.1 产品定位：AI 公司元框架（权威表述）

**〔需求〕来源：用户 2026-09-17/18 授权定义。** 这是 SoloIPs 的**产品定位权威正文**；其他文档（含 `AGENTS.md` 的项目红线）引用本条，**不复述为第二口径**。

> **SoloIPs 是一个 AI 公司元框架。三方分工如下：**
>
> | 方 | 提供什么 |
> | --- | --- |
> | **用户** | **runtime + 资源**——模型额度/供应商、创作工具与素材、算力与订阅、运营所需的外部渠道 |
> | **框架（SoloIPs）** | **结构 + 身份 + 任务协议**——公司/部门/团队的组织结构；员工与任职的稳定身份；任务如何委托、领取、审核与接续的协议 |
> | **员工（AI）** | **自主驱动**——在给定结构、身份与协议内自行规划与执行，而不是逐步接受人工指令 |

**三条推论**〔需求〕：

1. **框架不代替员工思考。** 框架提供的是承载工作的结构，不是预先编排好的工作流；把员工降为「执行固定步骤的脚本」偏离本定位。
2. **框架不持有用户的资源。** 模型额度、工具与素材、订阅与渠道归用户；框架只提供使用它们的结构（模型与凭据留在部署配置，见 §6「业务模块不读取个人绝对路径或凭据」与项目红线 2）。
3. **员工的自主不等于无约束。** 自主驱动运行在框架给出的身份与任务协议内；准入、撤权、审核与恢复等门禁仍是框架职责。

**与 §1 其余内容的关系**：上文的「研发团队 vs AI 公司」区分、SOLO-01/02/03、以及三层公司生态与个人 IP 公司平台定位，都是本定位在具体交付面的展开；它们不改变本定位，也不被本定位取消。

**引用约定**〔约束〕：**产品定位的维护正文是本条**（`docs/architecture.md` §1.1）。`AGENTS.md`「技术自主决策」小节把本条内容列为**不得偏离的平台核心方向之一**（连同三层公司生态、个人 IP 公司平台定位）——那是**操作规范层的引用**，不是独立定义。

**待同步**〔待办〕：`AGENTS.md` 当前以相同文字复述该定位，**尚未加上指向本节的引用**。按「单一正文、投影不复制」原则，`AGENTS.md` 后续应改为引用本正文。该同步待下次 AGENTS.md 写入时一并完成（本节登记，不自行改他文件）。

**因此**：定位变更须改本正文，再同步各引用方；不得只在 `AGENTS.md` 单方面改写。
...
| 工作台 | 输入目标、观察工作、回答问题、查看/退回/接受成果 | 首版使用 DSH Web 插槽、页面或侧栏；后续独立界面消费同一业务接口（V1 = 本仓自建最小插件 `soloips-web`，见 [`docs/decisions/web-ui-fork.md`](decisions/web-ui-fork.md)） |
...
| 日期 | 变更 | 变更者 |
| --- | --- | --- |
| 2026-09-18 | 新增 §1.1「产品定位：AI 公司元框架（权威表述）」：登记用户授权的三方分工定义（用户供 runtime+资源／框架供结构+身份+任务协议／员工自主驱动）、三条推论与引用约定；消除 `AGENTS.md` 作为该口径唯一出处的落差 | 文档智能体 |
